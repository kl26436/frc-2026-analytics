...
import { Save, Download, AlertCircle, CheckCircle, RotateCcw } from 'lucide-react';
...
function TBASettings() {
  ...
  const handleResetEvent = () => {
    ...
    // Initialize new pick list with the current event
    initializePickList(event);
        ...
        <div className="space-y-4">
          <div className="flex gap-3">
            ...
            <button
              ...
              className="flex items-center gap-2 px-6 py-3 bg-danger text-white rounded-lg hover:bg-danger/90 transition-colors font-semibold"
            >
              <RotateCcw size={20} />
              Reset Event
            </button>
          ...
          <p className="text-sm text-textSecondary">
            <strong>Load Rankings:</strong> Fetches current event rankings and adds the top 12 teams to your "Potatoes" tier.
          </p>
          <p className="text-sm text-textSecondary">
            <strong>Reset Event:</strong> Clears all pick list data, team selections, and reloads mock data for a fresh start.
          </p>
        </div>
      </div>
